!(function (e) {
    ...
    $("body").on("click", ".mobile-menu-bar", function (event) {
        event.preventDefault();
        $(".sidebar-menu-overlay").fadeIn();
        $(".side-menu-mobile").fadeIn();
    });
    $("body").on("click", ".close-mobile-menu", function (event) {
        event.preventDefault();
        $(".side-menu-mobile").fadeOut();
        $(".sidebar-menu-overlay").fadeOut();
    });
    $("body").on("click", ".sidebar-menu-overlay", function (event) {
        event.preventDefault();
        $(".sidebar-menu-overlay").fadeOut();
        $(".side-menu-mobile").fadeOut();
    });

    $('body').on('click','.buynow-single',function(event){
        event.preventDefault();
        $('.check-type-form').val('buynow');
        // setInterval(function () {
            $(".addtocart-form-btn").click();
        // }, 1000);
    });

    $('body').on('focusout','.input-email-check',function(event){
        var email = $(this).val();
        var url = base_url+'/check/auth/user/status';
        $.ajax({ url: url, type: "POST", data: {email:email} })
            .done(function (result) {
                if(result.status == 'success'){
                    alert('You are registered with this email id. Please login');
    ...
    $("body").on("click", ".mobile-menu-nav .has-children a.has-item", function (event) {
        event.preventDefault();
        ...
        } else if ($(this).children(".fa").hasClass("fa-minus")) {
    ...
    $("body").on("click", ".expand-list>.plus-sign", function (event) {
        event.preventDefault();
        $(this).toggleClass("open");
        $(this).parent(".expand-list").siblings(".filter-list").toggleClass("show", 50, "easeOutSine");
    });
    $("body").on("click", ".filter-title", function (event) {
        event.preventDefault();
        $(this).children("span.plus-sign").toggleClass("open");
        $(this).siblings(".filter-content").toggle("show");
    });
    $("body").on("change", '.filter-list>li>p>input[name="category"]', function (event) {
        event.preventDefault();
        $(".filter-list>li>p>input").each(function (index, el) {
    ...
    $("body").on("change", '.filter-list>li>p>input[name="brand"]', function (event) {
        event.preventDefault();
        filterData();
    });
    $("body").on("click", ".filter-price-btn", function (event) {
        event.preventDefault();
        filterData();
    });
    ...
    $("body").on("click", ".clear-brand-filter", function (event) {
        event.preventDefault();
        $('.filter-list>li>p>input[name="brand"]').each(function (index, el) {
            $(this).removeClass("checked");
            $(this).prop("checked", !1);
        });
        filterData();
    ...
    function filterData() {
        var min = $('.filter-content-price input[name="min"]').val();
        var max = $('.filter-content-price input[name="max"]').val();
        var query = "?page=1";
        if (min) query += "&min=" + min;
        if (max) query += "&max=" + max;
        var sortby = $("option:selected", ".product-sort-by select").val();
        if (sortby) query += "&sort-by=" + sortby;
        var brand = "";
        $('.filter-list>li>p>input[name="brand"]').each(function (index, el) {
            if ($(this).is(":checked")) brand = $(this).val();
        });
    ...
    e(function () {
        ...
        e(function () {
            ...
            if (
                ...
                e(".slide-bg-image, .bg-image").each(function (i) {
                    e(this).attr("data-background-img") && e(this).css("background-image", "url(" + e(this).data("background-img") + ")"),
                        e(this).attr("data-bg-position-x") && e(this).css("background-position", e(this).data("bg-position-x"));
                }),
                ...
                e(".nlpopup_close, #nlpopup_overlay").on("click", function (i) {
                    e.cookie("nlpopup", "closed", { expires: l, path: "/" }), e("#nlpopup, #nlpopup_overlay").fadeOut(200), i.preventDefault();
                }),
            ...
            c.on("click", function (i) {
                ...
                e(".color-selector .entry").on("click", function () {
                    ...
                    // alert(color_name+' '+pid);
                    var img_url = '/public/assets/products/images/';
                    var imageAll =[];
                    $.ajax({
                        url: base_url + "/get-product/color-variation",
                        type: "get",
                        data: {product_id: pid,color_name:color_name },
                        dataType: 'json',
                    })
                    .done(function(succ) {
                        console.log(succ.response);
                        if(succ){
                            var holdImg   = succ.response.images;
                            var imageAll = holdImg.split(',');
                            var dd__ = JSON.parse( holdImg );
                        }

                        console.log(holdImg);
                        
                        $("#color_varient").val(succ.response.color);
                        $("#color_su_code").val(succ.response.su_code);
                        
                        $.each(dd__,function(index, el) {
                            console.log('image-'+el);
                            let image = ['<div><img class="selectedImage" src="'+base_url+img_url+el+'"/></div>'].join('');
                            $(".main-img-slider").append(image);                       
                        });
                        ...
                        $.each(dd__,function(index, el) {
                            let image = ['<li><img class="lazyload" src="'+base_url+img_url+el+'" alt="image 1" /></li>'].join('');
                            $(".thumb-nav").append(image);                       
                        });
        ...
        e(function () {
            e(".product-item-5").owlCarousel({
                ...
                responsive: { 0: { items: 2 }, 320: { items: 2 }, 480: { items: 4 }, 775: { items: 4 }, 991: { items: 5 }, 1170: { items: 5 } },
            }),
                e(".product-item-4").owlCarousel({
                    ...
                    nav: !0,
                    navText: ['<i class="fa fa-angle-left" aria-hidden="true"></i>', '<i class="fa fa-angle-right" aria-hidden="true"></i>'],
                    responsive: { 0: { items: 2 }, 320: { items: 2 }, 480: { items: 2 }, 775: { items: 2 }, 991: { items: 3 }, 1170: { items: 4 } },
                }),
                e(".mobile-category-carousel").owlCarousel({
                    items: 3,
                    loop: !0,
                    margin: 5,
                    autoplay: !1,
                    ...
                    nav: !1,
                    navText: ['<i class="fa fa-angle-left" aria-hidden="true"></i>', '<i class="fa fa-angle-right" aria-hidden="true"></i>'],
                    responsive: { 0: { items: 3 }, 320: { items: 3 }, 480: { items: 5 }, 775: { items: 5 }, 991: { items: 3 }, 1170: { items: 5 } },
                }),
            ...
            i.slick({ dots: !1, fade: !0, slidesToShow: 1, slidesToScroll: 1, adaptiveHeight: !0, asNavFor: a, infinite: !1 }),
                ...
                e(".product-image-slider-thumbnails").find("button").hasClass("slick-arrow")
                    ? (e(".product-image-slider-thumbnails").css("padding-left", "30px"), e(".product-image-slider-thumbnails").css("padding-right", "30px"))
                    : (e(".product-image-slider-thumbnails").css("margin-left", "-7px"), e(".product-image-slider-thumbnails").css("margin-right", "-7px"));
        });
})(jQuery);
...
$(document).ready(function () {
    ...
    $("body").on("click", "#addToWishlist", function (event) {
        ...
        // _show();
        console.log('add and remove wishlist-'+$("#countTip").text());
        var $this = $(this);
        ...
        $.ajax({
            ...
            success: function (data) {
                if ($this.children(".fa").hasClass("fa-heart-o")) {
                    ...
                    $('.countTip--1').css('display','inline-block');
                    // $(".countTip--1").text(parseInt(1));
                } else if ($this.children(".fa").hasClass("fa-heart")) {
                    $this.children(".fa").removeClass("fa-heart");
                    $this.children(".fa").addClass("fa-heart-o");
                    // $("#countTip").text(parseInt($("#countTip").text()) - 1);

                    if($(".countTip--1").text() >= 1){
                        $(".countTip--1").text(parseInt($(".countTip--1").text()) - 1);
                        if(parseInt($(".countTip--1").text()) == 0){
                            $('.countTip--1').css('display','none');
                        }
                    }
                }
                // _hide();
    ...
    $("body").on("click", "button#btnDeleteWishlistItem", function (event) {
        ...
        var product_id = $this.attr("data-value");
        if (!id || !product_id) return !1;
        $.ajax({ url: base_url + "/add/wishlist", type: "POST", data: { id: id, product_id: product_id }, cache: !1 })
            .done(function (data) {
                $("#countTip").text(parseInt($("#countTip").text()) - 1);
                if($(".countTip--1").text() > 1){
                    $(".countTip--1").text(parseInt($(".countTip--1").text()) - 1);
    ...
    $("body").on("submit", ".product-add-to-cart-form", function (event) {
        ...
        if (color !== "" && color !== undefined) form = form + "&color=" + color;
        if (size !== "" && size !== undefined) form = form + "&size=" + size;
        if (c_size !== "" && c_size !== undefined) form = form + "&c_size=" + c_size;
        _show();
        var url = base_url + "/cart/store";
        if($('.check-type-form').val() =='buynow'){
            url = base_url + "/cart/store/buynow";
        }else{
        ...
        $.ajax({ url: url, type: "POST", data: form })
            .done(function (data) {
                console.log(data);
    ...
    $("body").on("click", "#addToCartProduct", function (event) {
        ...
        // var qty = 0;
        // if( $(this).closest('.single-cart-form').length > 0 ) {
        //     qty = $('.single-cart-form input[name="quantity"]').val();
        // }

        // var getQtyClass = $this.attr("data-qty");
        // var finalQtyInputFild =$("#"+getQtyClass).val();
        

        // var quantity = 1;
        // if( !finalQtyInputFild || qty > finalQtyInputFild ) {
        //     quantity = qty;
        // }
        // else {
        //     quantity = finalQtyInputFild;
        // }

        // var variations = [];
        // if( $('.product-variations').length > 0 ) {
            
        //     $('.product-variations input.qty').each(function(index, el) {
        //         var qty = $(this).val();
        //         var vid = $(this).siblings('input.pid').val();
        //         variations.push({vid:vid, qty:qty});
        //     });
        // }

        if (!id || !product_id) return !1;
        carTotalAmount.text(0);
        $.ajax({ url: base_url + "/cart/store", type: "POST", data: { id: id, product_id: product_id } })
            .done(function (data) {
                console.log(data);
                getCartProduct();
            })
    ...
    var getCartProduct = function () {
        $.ajax({ url: base_url + "/cart/get", type: "POST" })
            .done(function (data) {
    ...
    var udpateCartSidebar = function (data) {
        ...
        $.each(data, function (index, val) {
            ...
            var title = variation.variation_name ? variation.title + ' - ' + variation.variation_name : variation.title;
            ...
            $html += '<a class="product-link" href="' + variation.url + '">' + title + "</a>";
    ...
    var updateCart = function (form) {
        $.ajax({ url: base_url + "/cart/update", type: "POST", cache: !1, data: form })
            .done(function (data) {
                if (data.response === !1) {
    ...
    var updateCartTable = function (data) {
        ...
        $.each(data, function (index, val) {
            ...
            if (!amount) amount = variation.original_price;
            if (amount && variation.discount) amount = amount - (amount * variation.discount) / 100;
            if (variation.tax) amount = amount + (amount * variation.tax) / 100;
    ...
    $("body").on("submit", "form#checkAvailability", function (event) {
        ...
        $.ajax({ url: url, type: "POST", data: form })
            .done(function (result) {
                ...
                if (result.msg) {
                    $('<span class="error text-success">' + result.msg+ ' Estimated delivery: '+ result.pincode.delivery_time + "</span>").insertAfter($(".input-group", $this));
                }
                _hide();
            })
            .fail(function (error) {
    ...
    $("body").on("submit", "form#checkAvailability_home", function (event) {
        event.preventDefault();
        event.stopImmediatePropagation();
        event.stopPropagation();
        _show();
        ...
        var form = $this.serialize();
        var url = $this.attr("action");
        var pincodetd = $("input[name='pincode']").val();
        $.ajax({ url: url, type: "POST", data: form })
            .done(function (result) {
                createItem();
                if (result.error) {
                    $('<span class="error text-warning">' + result.error + "</span>").insertAfter($this);                    
                }
                if (result.msg) {
                    $('<span class="error text-success">' + result.msg + "</span>").insertAfter($this);
                    localStorage.setItem('td-pincode',pincodetd);
                    setTimeout( function(){ 
    ...
    if(localStorage.getItem('td-pincode')){
        $('#checkAvailability input[name="pincode"]').val(localStorage.getItem('td-pincode'));
    }
    ...
    var get_state = function (country_id) {
        $.ajax({ url: base_url + "/get/state/" + country_id, type: "GET" })
            .done(function (result) {
    ...
    if (state_id) get_city(state_id);
    function get_city(state_id) {
        $.ajax({ url: base_url + "/get/city/" + state_id, type: "GET" })
            .done(function (result) {
                html = '<option value="">Select</option>';
                if (result.cities) {
                    $.each(result.cities, function (index, val) {
                        if (city_id == this.id) html += '<option selected value="' + this.id + '">' + this.name + "</option>";
                        else html += '<option value="' + this.id + '">' + this.name + "</option>";
                    });
    ...
    $("body").on("click", "a#tabReview", function (event) {
        ...
        $("html, body").animate({ scrollTop: $("#otherDetails").offset().top }, 2000);
    });
    ...
    $("body").on("click", "button#applyCoupon", function (event) {
        ...
        _show();
         _hide();
        // $.ajax({ url: base_url + "/validate/code", type: "POST", data: { code: code, amount: amount } })
        //     .done(function (data) {
        ...
        //             if (coupon.discount_type === "percent") html += coupon.discount + "%";
    ...
    $("body").on("click", ".bmcoupon-apply", function (event) {
        ...
        if ($(this).siblings("input").is(":checked")) {
            if (totalAmt) {
                ...
                $(this).siblings("input").attr("checked", !1);
            }
        } else {
            if (totalAmt) {
                var amt = totalAmt - (totalAmt * 10) / 100;
    ...
    $("body").on("submit", "form#contactForm1", function (event) {
        ...
        $.ajax({ url: $url, type: "POST", data: form })
            .done(function (data) {
                ...
                if (data.mobile) {
                    $('<span class="text-danger">' + data.mobile[0] + "</span>").insertAfter($('input[name="mobile"]', $this));
                }
                if (data.message) {
        ...
        $.ajax({ url: url, type: "GET" })
            .done(function (data) {
                ...
                if (data.products && data.products.length > 0) {
                    appendRows(data.products, $("#loadMoreRow")); /*scrollBack()*/
                }
            })
            .fail(function (error) {})
            .always(function () {
    ...
    $("body").on("click", "button#loadMoreBtnHome", function (event) {
        ...
        event.preventDefault();
        $.ajax({ url: base_url + "/load/more?page=" + page, type: "GET" })
            .done(function (data) {
                console.log(data);
    ...
    var scrollBack = function () {
        $("html, body").animate({ scrollTop: $("#loadMoreRow").offset().top }, 1000);
    };
    var owl = $(".owl-carousel.main-carousel");
    owl.owlCarousel({ loop: !0, margin: 0, navSpeed: 1500, nav: !0, navText: ['<i class="fa fa-long-arrow-left" aria-hidden="true"></i>', '<i class="fa fa-long-arrow-right" aria-hidden="true"></i>'], autoplay: !0, rewind: !0, dots: !1, items: 1 });
    ...
    $("body").on("change", ".payment_method_cod", function (event) {
        ...
        _show();
        var dd = $(this);
        $("span.text-default").remove();
        var $this = $(this).attr('data-title');
        if($this =='online-payment'){
            
            $('.type-onlinepayment').removeClass('display--none');
            $('.type-onlinepayment').addClass('display--block animate__animated animate__fadeInDown');

            $('.type-card').removeClass('display--block');
            $('.type-upi').removeClass('display--block');
            $('.type-netbanking').removeClass('display--block');

            $('.type-card').addClass('display--none');
            $('.type-upi').addClass('display--none');
            $('.type-netbanking').addClass('display--none');

        }else if($this =='net-banking'){
            $('.type-netbanking').removeClass('display--none');
            $('.type-netbanking').addClass('display--block animate__animated animate__fadeInDown');

            $('.type-card').removeClass('display--block');
            $('.type-upi').removeClass('display--block');
            $('.type-onlinepayment').removeClass('display--block');

            $('.type-card').addClass('display--none');
            $('.type-upi').addClass('display--none');
            $('.type-onlinepayment').addClass('display--none');

        }else if($this =='credit-card'){

            $('.type-card').removeClass('display--none');
            $('.type-card').addClass('display--block animate__animated animate__fadeInDown');
        ...
        }else if($this =='pay-with-upi'){

            $('.type-upi').removeClass('display--none');
            $('.type-upi').addClass('display--block animate__animated animate__fadeInDown');
        ...
        //     $("#membership").attr("disabled", !0);
        // } else {
        //     $("#checkoutTotalAmount").text("");
        //     html = '<input type="hidden" id="totalCartAmount0" name="totalAmount" value="' + totalAmount + '">';
        //     html += '<i class="fa fa-inr"></i> <span>' + totalAmount + "</span>";
        //     $(html).appendTo("#checkoutTotalAmount");
        //     $("#membership").attr("disabled", !1);
        // }
    ...
    function get_attr_price(id, type, product_id) {
        console.log('dd');
        _show();
        $.ajax({ url: base_url + "/api/-/attribute/price", type: "POST", data: { id: id, type: type, pid: product_id } })
            .done(function (data) {
                console.log(data);

                if(data.size_image !==''){
                    console.log('fond'+data.size_image);
                    var holdImg   = data.size_image;
                    var dd__ = JSON.parse( holdImg );
                    var img_url = '/public/assets/products/images/';                
                    $.each(dd__,function(index, el) {
                        if(el){
                            let image = ['<div><img class="selectedImage  dd--remove" src="'+base_url+img_url+el+'"/></div>'].join('');
                            $(".main-img-slider").append(image);
                        }
                    });
                    ...
                    $.each(dd__,function(index, el) {
                        if(el){
                            let image__1 = ['<li><img class="lazyload" src="'+base_url+img_url+el+'" alt="image 1" /></li>'].join('');
                            $(".thumb-nav").append(image__1);
                        }
                    });
                ...
                if (data.price) {
                    console.log('d'+data.mrp);
                    var mrp_d = $('.get-product-mrp').attr('title');
                    html = "";
                    var price = data.price;
                    if (data.discount != 0) {
                        html += '<del><span class="fa fa-inr"></span> ' + Math.round(mrp_d) + "</del>";
                    ...
                    if (data.shipping_charge) {
                        html += '<span class="shipping-charge"> + <span class="fa fa-inr"></span>' + data.shipping_charge + " (Shipping charge)</span>";
                            ...
                            html += '<span class="shipping-charge"> +' + + " (Free Shipping)</span>";      
                        }
                    html + "</span>";
    ...
    $(".product-item-6").owlCarousel({
        ...
        nav: !0,
        navText: ['<i class="fa fa-angle-left" aria-hidden="true"></i>', '<i class="fa fa-angle-right" aria-hidden="true"></i>'],
        responsive: { 0: { items: 2, margin: 10 }, 320: { items: 2, margin: 10 }, 480: { items: 2, margin: 15 }, 775: { items: 3, margin: 20 }, 991: { items: 5, margin: 30 }, 1170: { items: 6, margin: 30 } },
    });
    ...
    $("body").on("change", "#membership", function (event) {
        ...
        } else totalAmount = totalAmount - amount;
    ...
    $("body").on("change", "input#document", function (event) {
        ...
        if (!file.name) return;
        _show();
        $.ajax({ url: base_url + "/seller/store/document", type: "POST", data: fd, contentType: !1, processData: !1 })
            .done(function (data) {
                if (data.validation == "failed") {
                    if (data.errors) {
                        if (data.errors.avtar) _error(data.errors.avtar, $this);
                        if (data.errors.pancard) _error(data.errors.pancard, $this);
                        if (data.errors.aadhar) _error(data.errors.aadhar, $this);
                        if (data.errors.signature) _error(data.errors.signature, $this);
                        if (data.errors.passbook) _error(data.errors.passbook, $this);
                        if (data.message) _error(data.message, $this);
                    }
                }
    ...
    $("body").on("click", ".mobile-filter-menu", function (event) {
        ...
        $(this).parent(".filter-button").siblings(".filter-sidebar").addClass("show animate__animated animate__fadeInLeft");
        if ($(this).children("span").hasClass("fa-filter")) {
            $(this).children("span").removeClass("fa fa-filter");
            $(this).children("span").addClass("close");
        } else {
            $(this).parent(".filter-button").siblings(".filter-sidebar").removeClass("show animate__animated animate__fadeInLeft");
    ...
    $("body").on("submit", "form#orderSearchForm", function (event) {
        ...
        $("#orderDetails").remove();
        _show();
        $.ajax({ url: $(this).attr("action"), type: "POST", data: form })
            .done(function (data) {
    ...
    $("body").on("change", 'input[name="return_type"]', function (event) {
        event.preventDefault();
        if ($(this).val() === "refund") {
            $(".customer-account-details").css({ display: "block" });
        } else $(".customer-account-details").css({ display: "none" });
    });
    $("body").on("submit", "form#orderComplainForm", function (event) {
        event.preventDefault();
        var $this = $(this);
        $("._error", this).remove();
        $(".response-text").text("");
        _show();
        $.ajax({ url: $(this).attr("action"), type: "POST", data: $(this).serializeArray() })
            .done(function (result) {
                if (result.errors) {
                    if (result.errors.return_type) {
                        _error(result.errors.return_type, $(".complain-type"));
                    }
                    if (result.errors.acc_holder_name) {
                        _error(result.errors.acc_holder_name, $('input[name="acc_holder_name"]'));
                    }
                    if (result.errors.account_no) {
                        _error(result.errors.account_no, $('input[name="account_no"]'));
                    }
                    if (result.errors.bank_name) {
                        _error(result.errors.bank_name, $('input[name="bank_name"]'));
                    }
                    if (result.errors.ifsc_code) {
                        _error(result.errors.ifsc_code, $('input[name="ifsc_code"]'));
                    }
                    if (result.errors.remark) {
                        _error(result.errors.remark, $('textarea[name="remark"]'));
                    }
                    if (result.errors.reason) {
                        _error(result.errors.reason, $('select[name="reason"]'));
                    }
                    if (result.errors.order_id) {
                        _error(result.errors.order_id, $(".response-text"));
                    } else if (result.errors.order_no) {
                        _error(result.errors.order_no, $(".response-text"));
                    } else if (result.errors.error) {
                        _error(result.errors.error, $(".response-text"));
                    }
    ...
    $(window).scroll(function() {    
        ...
        if (scroll >= 100) {
            $(".show-scroll").removeClass("none-dd");
            $('.sidebar-right.sidebar-open').css('top', '32px');
            $('.sidenav').css('top','25px');
        } else {
            $(".show-scroll").addClass("none-dd");
            $('.sidebar-right.sidebar-open').css('top', '70px');
            $('.sidenav').css('top','0');
        }
    });
    ...
    if(x==null){
        ...
        $('.newsletter-popup-area-section').addClass('active-newsletter-popup');    
        $('.newsletter-popup-area-section').removeClass('newsletter-popup');    
    }else{
        $('#newsletter-popup').modal('hide'); 
        $('.newsletter-popup-area-section').addClass('newsletter-popup');    
        $('.newsletter-popup-area-section').removeClass('active-newsletter-popup');    
    }
    ...
    if (window.location.hash) {
        $(".product-content-tabs li a").removeClass("active");
        $(".product-content-tabs li a.review").addClass("active");
        $(".product-content-Tabs_wraper .tab-pane").removeClass("active show in");
        $(".product-content-Tabs_wraper .reviews.tab-pane").addClass("active show in");
        $("html, body").animate(
            {
