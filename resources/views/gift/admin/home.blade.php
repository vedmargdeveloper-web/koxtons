                            ...
                            @foreach( $topProducts as $pro )
                                ...
                                @if( $product && isset($product->product_category[0]->slug) )
                                    <div class="d-flex flex-column flex-sm-row align-items-sm-center mb-3">
                                        <img style="width:60px;" class="avatar-lg mb-3 mb-sm-0 rounded mr-sm-3" src="{{ asset( 'public/'. product_file( thumb( $product->feature_image, config('filesize.thumbnail.0'), config('filesize.thumbnail.1') ) ) ) }}" alt="{{ $product->feature_image_alt ?? $product->title }}">
