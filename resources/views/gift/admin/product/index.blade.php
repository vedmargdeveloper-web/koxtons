...
<?php $products = App\model\Product::with(['review', 'user.products', 'product_category'])->orderby('id', 'DESC')->get(); ?>
	...
	<div class="row">
		<div class="card">
			<div class="card-header">
				...
				<a href="{{ route('product.create') }}"><i class="fa fa-plus"></i></a>
			</div>
					...
					<tbody>
						...
						@if( $products )
							...
							<tr>
								...
								{{-- <td>
									@if( $row->user && count($row->user) > 0 )
										
										<a href="{{ route('admin.seller.show', $row->user[0]->id) }}">{{ ucfirst($row->user[0]->first_name).' '.ucfirst($row->user[0]->last_name) }}</a>
								...
								<td>
									@if( $row->user && count($row->user) > 0 )
										...
										{{ isset( $row->user[0]->products ) ? $row->user[0]->products->count() : 0 }}
										
									@endif
								</td> --}}
								<td>
									@if( $row->review->count() > 0 )
										<a href="{{ route('admin.product.review', $row->id) }}">
											{{ $row->review->count() }}
										</a>
									@endif
								</td>
								<td>
									<span class="mystatus-<?=$row->id?>">{{ ucfirst($row->status) }}</span>
